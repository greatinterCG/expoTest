	...
	this.frame_0 = function() {
		...
		function calculateFling() {
		    ...
		    if (flingDuration < 0.5) {
		        flingXFinal = (flingX < 0) ? Math.max(0, draggableObject.x + flingX * 50) : Math.min(MaxX, draggableObject.x + flingX * 50);
		        flingYFinal = (flingY < 0) ? Math.max(0, draggableObject.y + flingY * 50) : Math.min(MaxY, draggableObject.y + flingY * 50);
		    } else {
		        flingXFinal = (flingX < 0) ? Math.max(0, draggableObject.x + flingX * 10) : Math.min(MaxX, draggableObject.x + flingX * 10);
		        flingYFinal = (flingY < 0) ? Math.max(0, draggableObject.y + flingY * 10) : Math.min(MaxY, draggableObject.y + flingY * 10);
		        flingDuration = 1;
		    }
