...
lib.ssMetadata = [
		{name:"スクロールサンプル_atlas_1", frames: [[0,1336,1256,203],[1739,201,174,214],[1258,1015,174,214],[1484,0,253,218],[1434,1015,174,214],[0,0,1482,280],[0,282,1482,280],[1610,1015,174,214],[1484,220,253,218],[0,772,1151,280],[0,1054,1151,280],[1153,772,800,241],[1739,0,238,199],[1786,1015,152,152],[0,564,1989,206],[0,1541,1954,88]]}
];
...
if (reversed == null) { reversed = false; }
	...
	this.instance_12 = new lib.CachedBmp_171();
	...
	this.instance_14 = new lib.CachedBmp_164();
	...
	this.instance_18 = new lib.CachedBmp_171();
...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		function drag(event) {
		    ...
		    // デバッグ情報をテキストボックスに表示
		    updateLog(xCoords, yCoords, tempNewX, tempNewY);
		...
		function stopDrag(event) {
		    // 速度を計算
		    if (xCoords.length >= 3) {
		        velocityX = (xCoords[xCoords.length - 1] - xCoords[xCoords.length - 3]) / 2;
		        velocityY = (yCoords[yCoords.length - 1] - yCoords[yCoords.length - 3]) / 2;
		    } else {
		        velocityX = 0;
		        velocityY = 0;
		    }
		...
		function handleTick(event) {
		    if (isDragging) {
		        ...
		        if (canScrollVertically) {
		            ...
		            tempNewY = draggableObject.y; // 垂直方向のスクロールが禁止されている場合、Y座標を変更しない
		        }
		        ...
		        if (canScrollHorizontally) {
		            ...
		            tempNewX = draggableObject.x; // 水平方向のスクロールが禁止されている場合、X座標を変更しない
		        }
		        ...
		        if (canScrollVertically) {
		            ...
		            } else if (newY + objectHeight < MaxY) {
		                newY = MaxY - objectHeight;
		                velocityY = 0; // 境界に達したら速度をリセット
		            }
		        }
		        ...
		        if (canScrollHorizontally) {
		            ...
		            } else if (newX + objectWidth < MaxX) {
		                newX = MaxX - objectWidth;
		                velocityX = 0; // 境界に達したら速度をリセット
		            }
		        }
		
		        // オブジェクトの位置を更新
		        draggableObject.x = newX;
		        draggableObject.y = newY;
		        ...
		        // 現在の座標を保存
		        saveCoords(newX, newY);
		    }
		}
		
		// ログの更新
		function updateLog(xCoords, yCoords, newX, newY) {
		    if(showDebug) {
		        logbox.text = "xCoords: " + xCoords + "\ryCoords: " + yCoords + "\rnewX: " + newX + "\rnewY: " + newY + "\r";
		    }
		}
...
if (reversed == null) { reversed = false; }
	...
	this.instance_1.setTransform(125.85,33.55,0.5,0.5);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(2));

	// トークヘッダ
	this.instance_2 = new lib.CachedBmp_154();
	this.instance_2.setTransform(-0.35,0,0.5,0.5);

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(2));

	// トークフッター
	this.instance_3 = new lib.ClipGroup();
	this.instance_3.setTransform(360.85,2420.55,1,1,0,0,0,59.5,57.1);

	this.instance_4 = new lib.CachedBmp_157();
	this.instance_4.setTransform(320.75,2360.5,0.5,0.5);

	this.instance_5 = new lib.CachedBmp_174();
	this.instance_5.setTransform(86.2,2347,0.5,0.5);

	this.instance_6 = new lib.CachedBmp_155();
	this.instance_6.setTransform(-6.35,2322.05,0.5,0.5);
	...
	// スクロールペイン
	this.instance_7 = new lib.スクロールペイン();
	this.instance_7.setTransform(0,255.4);
